...
parser=argparse.ArgumentParser(description="add a windows directory to the path")
parser.add_argument("-input", required=True, help="Path to the download directory")
parser.add_argument("-output", required=True, help="Path to the upload directory")
...
def convert_files(file_paths, log_file):
    ...
    for file_path in file_paths:
        try:
            ...
            log_message = (
                f'"{file_name}" ({get_file_size(original_file_size)}) was converted to "{Path(output_file_path).name}\n" '
...
def main():
    ...
    log_file_name = f"{converted_folder}\{log_file_name}"
