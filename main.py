...
def scan_directory():
    ...
    for file_name in os.scandir("convert_media"):
        if file_name.is_file():
            if check_file_convertibility(file_name.path, log_messages):
...
def check_file_convertibility(file_path, log_messages):
    """
    ...
    try:
        ...
        log_messages.append(
            f'Error checking convertibility of "{file_path}":\n{error_output}\n'
        )
        return False
...
def convert_files(file_paths, log_file):
    ...
    for file_path in file_paths:
        try:
            ...
            error_output = errors.stderr.strip()
            log_messages.append(f'Error converting "{file_path}":\n{error_output}')
